...
MoveOrdering::MoveOrdering() { clear(); }
...
void MoveOrdering::clear() {
  ...
  std::fill(&captureHist_[0][0][0][0],
            &captureHist_[0][0][0][0] + static_cast<size_t>(Core::COLOR_NB) *
...
void MoveOrdering::update_killers(int ply, Core::Move move) {
  if (ply < 0 || ply >= MAX_PLY || !move.is_ok() || move.is_capture())
    return;
...
void MoveOrdering::update_history(Core::Color side, Core::Move move,
                                  int depth) {
  if (move.is_capture())
    return;
...
void MoveOrdering::update_history_malus(Core::Color side, Core::Move move,
                                        int depth) {
  if (move.is_capture())
    return;
...
void MoveOrdering::update_capture(Core::Color side, Core::PieceType attacker,
...
void MoveOrdering::update_capture_malus(Core::Color side,
                                        Core::PieceType attacker,
                                        Core::Square to, Core::PieceType victim,
                                        int depth) {
  int &score = captureHist_[side][attacker][to][victim];
...
int MoveOrdering::score_move(const Core::Position &pos, Core::Move move,
                             Core::Move ttMove, int ply) const {
  if (ttMove.is_ok() && move == ttMove)
...
void MoveOrdering::sort_moves(const Core::Position &pos, Core::MoveList &moves,
                              Core::Move ttMove, int ply) const {
