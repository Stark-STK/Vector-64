...
namespace Core {
namespace {
enum GenType : uint8_t { GEN_ALL, GEN_CAPTURES, GEN_QUIETS };

inline bool
is_square_attacked_by_enemy(Square sq, Color us, Bitboard occ,
                            Bitboard enemyPawns, Bitboard enemyKnights,
                            Bitboard enemyBishops, Bitboard enemyRooks,
                            Bitboard enemyQueens, Bitboard enemyKing) {
  if (Attacks::pawn_attacks(sq, us) & enemyPawns)
    return true;
  if (Attacks::knight_attacks(sq) & enemyKnights)
    return true;
  if (Attacks::king_attacks(sq) & enemyKing)
    return true;
  if (Attacks::bishop_attacks(sq, occ) & (enemyBishops | enemyQueens))
    return true;
  if (Attacks::rook_attacks(sq, occ) & (enemyRooks | enemyQueens))
    return true;
  return false;
}
...
void generate_pawn_pseudos(const Position &pos, MoveList &moves) {
  ...
  if constexpr (Gt != GEN_CAPTURES) {
    Bitboard single_push_on_3 =
        single_push & (Us == WHITE ? RANK_3_BB : RANK_6_BB);
...
void generate_piece_pseudos(const Position &pos, MoveList &moves) {
  ...
  while (pieces) {
    ...
    if constexpr (Pt == KNIGHT)
      attacks = Attacks::knight_attacks(from);
    else if constexpr (Pt == BISHOP)
      attacks = Attacks::bishop_attacks(from, occupancy);
    else if constexpr (Pt == ROOK)
      attacks = Attacks::rook_attacks(from, occupancy);
    else if constexpr (Pt == QUEEN)
      attacks = Attacks::queen_attacks(from, occupancy);
    else if constexpr (Pt == KING)
      attacks = Attacks::king_attacks(from);
...
NodeLegality make_node_legality(const Position &pos) {
  ...
  // occupancy, so a king move along a slider ray is scored correctly.
  {
    ...
    if (them == WHITE) {
      danger |=
          shift<NORTH_WEST>(nl.enemyPawns) | shift<NORTH_EAST>(nl.enemyPawns);
    } else {
      danger |=
          shift<SOUTH_WEST>(nl.enemyPawns) | shift<SOUTH_EAST>(nl.enemyPawns);
    }
    Bitboard b = nl.enemyKnights;
    while (b)
      danger |= Attacks::knight_attacks(pop_lsb(b));
    if (nl.enemyKing)
      danger |= Attacks::king_attacks(lsb(nl.enemyKing));
    b = nl.enemyBishops | nl.enemyQueens;
    while (b)
      danger |= Attacks::bishop_attacks(pop_lsb(b), occNoKing);
    b = nl.enemyRooks | nl.enemyQueens;
    while (b)
      danger |= Attacks::rook_attacks(pop_lsb(b), occNoKing);
...
void ensure_pins(const NodeLegality &nl) {
  ...
  Bitboard snipers =
      ...
      (Attacks::bishop_attacks(nl.kingSq, 0) &
       (nl.enemyBishops | nl.enemyQueens));
...
bool is_move_legal_full(const NodeLegality &nl, Move m) {
  ...
  if (m.is_capture()) {
    ...
    Square capSq = to;
    if (m.is_en_passant()) {
      capSq = make_square(static_cast<GenFile>(file_of(to)),
                          static_cast<GenRank>(rank_of(from)));
    }
  ...
  if (m.is_castling()) {
    ...
    if (is_square_attacked_by_enemy(from, nl.us, nl.occ, enemyPawns,
                                    enemyKnights, enemyBishops, enemyRooks,
                                    enemyQueens, enemyKing)) {
      return false;
    }

    const Square mid = static_cast<Square>((from + to) / 2);
    const Bitboard occMid = (nl.occ ^ fromBB) | square_bb(mid);
    if (is_square_attacked_by_enemy(mid, nl.us, occMid, enemyPawns,
                                    enemyKnights, enemyBishops, enemyRooks,
                                    enemyQueens, enemyKing)) {
      return false;
    }
    ...
    const Square rookFrom =
        (to > from) ? make_square(FILE_H, rank) : make_square(FILE_A, rank);
    const Square rookTo =
        (to > from) ? make_square(FILE_F, rank) : make_square(FILE_D, rank);
  ...
  return !is_square_attacked_by_enemy(kingSq, nl.us, occAfter, enemyPawns,
                                      enemyKnights, enemyBishops, enemyRooks,
                                      enemyQueens, enemyKing);
}
...
bool is_pseudo_legal(const Position &pos, Move m) {
  ...
  if (m.is_castling()) {
    ...
    if (us == WHITE) {
      if (from != SQ_E1)
        return false;
      if (to == SQ_G1)
        return (rights & 1) && has_bit(rooks, SQ_H1) && !has_bit(occ, SQ_F1) &&
               !has_bit(occ, SQ_G1);
      if (to == SQ_C1)
        return (rights & 2) && has_bit(rooks, SQ_A1) && !has_bit(occ, SQ_B1) &&
               !has_bit(occ, SQ_C1) && !has_bit(occ, SQ_D1);
      return false;
    }
    if (from != SQ_E8)
      return false;
    if (to == SQ_G8)
      return (rights & 4) && has_bit(rooks, SQ_H8) && !has_bit(occ, SQ_F8) &&
             !has_bit(occ, SQ_G8);
    if (to == SQ_C8)
      return (rights & 8) && has_bit(rooks, SQ_A8) && !has_bit(occ, SQ_B8) &&
             !has_bit(occ, SQ_C8) && !has_bit(occ, SQ_D8);
    return false;
  }
  ...
  if (pt == PAWN) {
    ...
    if ((rank_of(from) == promoFromRank) != m.is_promotion())
      return false;
  ...
  case KNIGHT:
    ...
    break;
  case BISHOP:
    attacks = Attacks::bishop_attacks(from, occ);
    break;
  case ROOK:
    attacks = Attacks::rook_attacks(from, occ);
    break;
  case QUEEN:
    attacks = Attacks::queen_attacks(from, occ);
    break;
  case KING:
    attacks = Attacks::king_attacks(from);
    break;
