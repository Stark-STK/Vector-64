...
Bitboard compute_sliding_attack(Square square, Bitboard occupancy,
                                bool is_bishop) {
  Bitboard attacks = 0;
...
Bitboard get_magic_mask(Square square, bool is_bishop) {
  ...
  if (is_bishop) {
    for (int r = r0 + 1, f = f0 + 1; r < 7 && f < 7; r++, f++)
      mask |= square_bb(make_square((GenFile)f, (GenRank)r));
    ...
    for (int r = r0 + 1; r < 7; r++)
      mask |= square_bb(make_square((GenFile)f0, (GenRank)r));
    for (int r = r0 - 1; r > 0; r--)
      mask |= square_bb(make_square((GenFile)f0, (GenRank)r));
    for (int f = f0 + 1; f < 7; f++)
      mask |= square_bb(make_square((GenFile)f, (GenRank)r0));
...
void init_magics(bool is_bishop, Bitboard *&attack_table_ptr) {
  ...
  for (int s = 0; s < 64; ++s) {
    ...
    if (attack_table_ptr + variation_count >
        AttackTable + MAX_ATTACK_TABLE_SIZE) {
      std::cerr << "CRITICAL ERROR: Attack Table Buffer Overflow!" << std::endl;
      exit(1);
    }
    for (int i = 0; i < variation_count; ++i)
      attack_table_ptr[i] = attacks[i];
    ...
    for (int k = 0; k < 1000000; ++k) {
      ...
      if (attack_table_ptr + variation_count >
          AttackTable + MAX_ATTACK_TABLE_SIZE) {
        std::cerr << "CRITICAL ERROR: Magic Table Buffer Overflow!" << '\n';
        exit(1);
      }
      ...
      for (int i = 0; i < variation_count; ++i) {
        ...
        if (idx >= (unsigned)variation_count) {
          fail = true;
          break;
        }
      ...
      if (!fail) {
        ...
        for (int i = 0; i < variation_count; ++i) {
          if (temp_used[i])
            attack_table_ptr[i] = temp_table[i];
        }
        attack_table_ptr += variation_count;
        found = true;
        break;
    ...
    if (!found) {
      std::cerr << "CRITICAL ERROR: Failed to find Magic for square " << s
                << '\n';
      exit(1);
    }
  }
